...
wget -q -O /etc/systemd/system/socks.service "${REPO}xray/socks.service" && chmod +x socks.service >/dev/null 2>&1
...
wget -q -O /usr/bin/ws.py "${REPO}ssh/ws.py" >/dev/null 2>&1
chmod +x /etc/xray/limit.vmess
chmod +x /etc/xray/limit.vless
chmod +x /etc/xray/limit.trojan
chmod +x /etc/xray/limit.shadowsocks
chmod +x /etc/usr/bin/ws.py
